...
& $Psql -h $PgHost -p $PgPort -U $PgUser -d postgres -tc "SELECT 1 FROM pg_roles WHERE rolname='$AppUser'" | Select-String -Quiet "1" | Out-Null
$roleExists = (& $Psql -h $PgHost -p $PgPort -U $PgUser -d postgres -tAc "SELECT 1 FROM pg_roles WHERE rolname='$AppUser'").Trim() -eq "1"
if (-not $roleExists) {
...
$dbExists = (& $Psql -h $PgHost -p $PgPort -U $PgUser -d postgres -tAc "SELECT 1 FROM pg_database WHERE datname='$DbName'").Trim() -eq "1"
if (-not $dbExists) {
