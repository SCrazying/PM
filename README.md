...
- [x] **V1.0.3**：工作台**月度日历视图**（最近进展按月选天，今日已填报可直接改）/ 项目详情**风险管理**（单独添加/删除/关闭）/ 周目标**任意周查看编辑**（周次下拉含前几周、动态并入有数据的周）/ **日历方式选周**（dayjs 周一起始）/ 全站弹窗加大 / 看板风险可点击跳项目 + 机型筛选 / 项目资料下载授权修复 / attachment.mime_type 修复（迁移 0013）/ 完成度与子节点保留修复
- [x] **V1.0.4**：**全面操作日志（审计）**——所有项目/人员操作（新增周目标、进展填报、风险、成员、任务、节点流转、附件、配置、用户、模板、导入、备份等 58 个写接口全覆盖）记录 `audit_log`；管理端**操作日志页**（按操作者/动作/对象/日期过滤、详情 tooltip、CSV 导出）；登录失败/锁定安全审计；日志**保留期自动清理**（后台每日任务）；恢复项目落库修复、批量回收站 target_id 溢出修复（迁移 0014 索引）
