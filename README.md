...
- [x] M7 项目状态手动配置（未开始/进行中/延期/已完成/暂停/归档，项目列表内联编辑）+ 健康度下线

> 全部里程碑已完成，端到端在真实 PostgreSQL 上验证通过。63 个 API 端点。

## 功能总览
| 模块 | 功能 |
|---|---|
| 项目看板 | 状态列（未开始/进行中/延期/已完成/暂停）+ 机型筛选 + 年/月视图 |
| 项目管理 | 新建/编辑/归档 + **项目状态手动配置（列表内联编辑）** + **机型下拉选择** + **立项自选 TR 节点** + 成员角色（**SE/TPM 单选、TL/FO/CodeReview 多选**） |
...
| 系统管理 | 用户 / **机型管理** / TR 模板 / **Excel 台账导入与导出** / 系统配置 / 数据备份 |
