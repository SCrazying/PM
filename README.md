...
> 全部里程碑已完成，端到端在真实 PostgreSQL 上验证通过（25/25）。52 个 API 端点。

## 功能总览
| 模块 | 功能 |
|---|---|
| 项目看板 | 状态列（未开始/进行中/延期/已完成/暂停）+ 机型筛选 + 健康度 |
| 项目管理 | 新建/编辑/归档 + **立项自选 TR 节点** + 成员管理（角色/是否投入） |
| 项目详情 | TR 节点进度条 + 节点任务看板 + **评审流转 + 整改闭环** + 周目标 + 进展时间线 |
| 工作台 | 今日待填报 + 项目级进展填报（多人/关联任务/风险） |
| 周会视图 | **按项目 / 按人 双视图** + 项目周报（周目标/本周任务/每日明细/风险） |
| 个人绩效 | 月/季/年工作汇总 + **AI 自动生成绩效总结**（可编辑/导出/看依据，无 AI 时模板降级） |
| 系统管理 | 用户 / TR 模板 / **Excel 台账导入** / 系统配置 / 数据备份 |
